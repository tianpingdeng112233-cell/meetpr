...
@Suite struct SessionReflectionStoreTests {
  private func makeStore() throws -> (UserDefaultsSessionReflectionStore, UserDefaults) {
  ...
  @Test func savesAndReloadsReflection() throws {
  ...
  @Test func unknownDayReturnsEmpty() throws {
    let (store, _) = try makeStore()
    let student = UUID()
  ...
  @Test func differentStudentsDoNotShareReflections() throws {
    let (store, _) = try makeStore()
  ...
  @Test func emptyReflectionClearsStoredRecord() throws {
    let (store, _) = try makeStore()
    let student = UUID()
    let day = Date(timeIntervalSince1970: 1_720_051_200)
  ...
  @Test func sameCalendarDayReloadsRegardlessOfTimeOfDay() throws {
    let (store, _) = try makeStore()
    let student = UUID()
