...
actor ImportedHistoryBackfill {
  ...
  private func makeReviewPlan(
    ...
    for (family, familyCandidates) in grouped {
      guard let maximum = familyCandidates.max(by: { $0.e1RMKg < $1.e1RMKg }) else { continue }
  ...
  /// Applies an answer only to the IDs captured by the pending review. A later
  /// higher batch creates a fresh pending review, so this cannot rewrite an
  /// older rejected batch (spec 053 §5).
  func answer(
...
extension ImportedHistoryBackfill {
    ...
    return allLogs.compactMap { log in
      ...
      guard E1RMEligibility.isEligible(reps: log.reps, rpe: sourceRPE, family: family),
        let e1RMKg = E1RMCalculator.calculate(
          weightKg: sourceWeightKg, reps: log.reps, rpe: sourceRPE)
      else { return nil }
      return Candidate(
        log: log,
  ...
  private func fetchWindow(studentID: UUID) async throws -> [StudentSetLog] {
    ...
    while lowerBound <= end {
      ...
      let logs = try await self.logs.fetchLogs(
        studentID: studentID,
      ...
      guard let next = calendar.date(byAdding: .second, value: 1, to: sliceEnd) else { break }
      lowerBound = next
    }
    return allLogs.values.sorted { $0.loggedAt < $1.loggedAt }
  }
  ...
  private func existingImportedPoints(
    ...
    return Dictionary(
      histories.values.flatMap { $0 }.filter { $0.origin == .imported }.map { ($0.setLogId, $0) },
...
private struct PendingPlan: Sendable {
  ...
  func makeReview(studentID: UUID, pointIDs: Set<UUID>) -> PendingImportedHistoryReview {
    let mergedIDs = Set(existing?.pointIDs ?? []).union(pointIDs)
    let useNewMaximum = maximum.e1RMKg > (existing?.reviewedMaxE1RM ?? -.infinity)
    return PendingImportedHistoryReview(
      id: existing?.id ?? UUID(),
      studentID: studentID,
      family: existing?.family ?? maximum.family ?? .squat,
      pointIDs: mergedIDs.sorted { $0.uuidString < $1.uuidString },
