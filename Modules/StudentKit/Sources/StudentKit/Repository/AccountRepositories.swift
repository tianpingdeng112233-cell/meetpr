...
/// Live account ledger (spec 048): DELETE /me + PUT /me/password.
public struct BackendAccountRepository: AccountRepository {
  private let api: APIClient
  private let session: SessionStateReader

  public init(api: APIClient, session: SessionStateReader) {
    self.api = api
...
/// Demo/preview account store: verifies against a seeded password and
/// records the deletion so tests can assert on it.
public actor InMemoryAccountRepository: AccountRepository {
  ...
  public func currentPassword() -> String { password }
}
