# 学员端 iOS 设计系统正典(2026-07-28 起生效)

⚖️ David 2026-07-28 拍板:**学员端黑金 v3 交付收尾;此后所有新设计一律按本设计系统来**。
新页面、新组件、新弹层、外部设计稿——先对照本文,不一致的以本文为准,或先经 David 拍板改本文。

## 0. 适用范围(⚖️ David 追加:「单指学员的 iOS 端」)

| 面 | 是否受本文约束 |
|---|---|
| **学员端 iOS**(`Modules/StudentKit` + 其消费的 `DesignSystem`) | ✅ 唯一约束对象 |
| 教练端 iOS(`CoachKit`)、登录/注册流 | ❌ 不受约束;仍用 v2 色板与恒暗,迁移是后续独立 wave 的事 |
| `ChatUI` 共享模块内部 | ❌ 不受约束(学员侧走自建黑金视图 `StudentBlackGoldChatView`,不改共享件外观) |
| plan-web 教练网页端 | ❌ 不受约束(墨蓝亮色体系,另有设计源 `docs/design/coach-web`) |
| backend / 其他仓 | ❌ 无关 |

跨端改动落到非学员面时,别拿本文当理由;要统一先经 David 拍板。
