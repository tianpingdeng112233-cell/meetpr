...
- **账号台账**(port #221):学员「我的」新增注销账号、修改密码与全量训练数据 CSV 导出
...
- **学员可见教练组备注**:训练组行与历史组记录都会显示教练写的备注,空备注不占位
- **[P0] 「我的」页退出登录始终可达**:资料未填/加载失败时也显示退出入口,账号不再被困在登录态

- **[P0] 组记录失败防扩散**:重量/次数/RPE 保存失败时保留训练页和刚输入的值,
  sheet 不再提前关闭;视频前置写组失败会明确提示,不再把整页替换成“加载失败”。
  - ~~staging 真环境仍 500~~ **已根治(2026-07-10 晚)**:staging 库补齐迁移 0031→0037 后
    curl 验证 `POST /sets/log` 500→201,记重量/录传视频真实写入已恢复(连线上 1.0(7) 一起修好);
