...
- build 号:22（2026-09-16 经 agvtool 单源更新；ASC 当前最高 21 已现场核实）
- 切包:annotated tag `beta/1.0-22` @ `07489315` 已推送；tag CI 全绿，双端 demo 已装 1.0(22)。专用取包树 `MeetPR-release-1.0-22`，代码侧可 Archive，尚未上传；Apple 新协议待 David 本人处理。证据与四基线见 [切包记录](docs/prep-beta-1.0-22-2026-09-16.md)。
...
David 批准的 #339 / #342 / #341 / #340 / #343 已全部进入 `release/1.0`。#339 原已落线，其余四项随整合 PR #344 于 2026-09-16 合入（9a28115d），原 PR 均为 MERGED。切包状态见上方；尚未部署配套后端。
...
- **部署与发布剩余项**：APNs key 已在 Bitwarden；最近线上预检（09-14）为 `sandbox`，TestFlight production 配置与实际送达未验收；0070 尚未在线上执行。本次切包进度见顶部与切包记录；Archive / Upload 由 David 手动。
