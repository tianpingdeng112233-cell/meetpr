...
- **spec 071 训练日推进制(学员端换制,T3)**(PR #314,2026-08-08 merge 合入 0f1b3b1)[P1]:
  游标制取代日期锚定:今日卡/训练 tab 以「第一个未完成日」为轴,推荐日期降级纯展示,
  顺延学员端触发链整删(净删 ~2200 行,展示字段/push 路由留 W2);序列进度条/周行卡
  计划汇总/三态横幅/完成态卡+撤销,视觉正典=docs/design/sequence-handoff 二审稿。
  ⚖️08-08 三连拍板:未开始态=汇总卡(0 组日任何入口不跳过,≥1 组重进恢复记录卡);
  **结算权归学员**(记满不自动结算,长按「完成今日训练」=唯一学员侧结算,backend #201
  已同步下线 auto 并部署 staging+真号实证);问教练=卡片右上角「问教练」文字芯片全程唯一入口。
  依赖:backend 035(0057+#197+#201)已全量上线 staging。闸门:review-loop 累计 10+8 BLOCKER
  全清多轮 CLEAN,703 测试绿,David 双机(demo+staging 真号)多轮真机走查亲验收口
  (2026-08-08「ok了没问题 进下班车」)。⚠️注意:配套 backend 已上线,老包(≤1.0(17))
  行为不受影响(顺延域冻结保留);但 071 上线后学员记满不再自动完成——教练侧感知
  依赖学员长按,W2 再做教练分诊重定义。
