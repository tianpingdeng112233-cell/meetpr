...
## 🎯 目标:1.0 (20)
- 基底:`beta/1.0-19`(= TestFlight 在测的 1.0(19),2026-08-13 上传)之后的 release/1.0 直推累积
- build 号:20(待 prep-beta 时 bump,agvtool 单源)
- 切包:本分支打 tag `beta/1.0-20`

## 本版将包含(落线后追加到这里)

- **[P0] 修「记录完成后消失」**(bdc9ab45):推进制下超过计划排期终点后,今日页/周历拉取
  计划内记录的日期窗口把新记的组滤掉,显示 0/N 已记录、重量看似丢失(服务端数据完好)。
  窗口上界改 max(末排期日, 今天)+1d。倪嘉骏 08-19/08-20 两报即此病,催其升级本包验证。

- **spec 074 Global 轨三通道登录(W4)**——仅 Global 构建轨生效,**CN 四配置产物逐字节零变化**
  (Release 产物实证:无 Google scheme/无 applesignin entitlement);随线不随包,CN 内测包行为不受影响。
