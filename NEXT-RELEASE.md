...
- solo 注册入口门出内测包(defer≠delete)(#230)

**2026-07-10 收尾列车(053 实装 + 加固批次 + 顺延,#227/#234)**
- 导入历史 e1RM 回填实装:置信度分级 + 复核门,永不触发 PR 庆祝/遥测(spec 053 impl,#227)
- 教练组备注在学员训练页/录入 sheet/历史展示;推定完成「按计划推定」徽章(#234)
- 学员本地数据迁入受保护 Application Support(一次性迁移不丢数据)(#234)
- 视频删除改服务端归属校验确认制,网络抖动不再假删(#234;**依赖 backend uploads DELETE/reconcile,backend 先部署**)
- 教练 per-set 逐组重量/次数目标(#234)
- 教练从学员资料页改名(#234)
- 动作别名 44→51 条(#234;6 条因 canonical 缺失搁置待拍板)
- **学员单日顺延**:coached 学员「今天有事」延到本周下个休息日,UTC 门控/当日可撤/教练时间线标记/triage 不误报(spec 015,#234;**依赖 backend 迁移 0037 + shift 端点,backend 先部署**)
- day_of_week 语义正典化:位置式=计划周第 N 天,三端对齐(#234 + backend #49-#51)
