...
- build 号:17(已 bump,2026-08-01 prep-beta)
...
- **RPE 刻度选档修复 + 拍摄按钮回首屏(PR #290,2026-08-02 合入 2234087)[P1]**:修「RPE 滚动老是
  选错」三缺陷(命中判定按真实竖条中心/抬手不重算/滚动横抖不误写),拖动加跟随气泡(拍板 B);
  852pt 机型杠铃图 148→108pt,拍摄/相册回到首屏。rebase 到 P0 修复之上重验:StudentKit 649 +
  DesignSystem 71 绿,模拟器点 7 落 7.0、拍摄按钮首屏可见。
- **杠铃卧推升格主项变式(PR #303,2026-08-02 合入 bfecee9,随 backend 0056)[P1]**:内置动作库
  杠铃卧推 accessory→main_lift_variation/bench(竞技卧推不动),进入主项及变式口径。
  后端迁移 0056 已应用并部署 staging(sha-2adbbeb,backend #188/#189),先后端后 iOS 顺序正确。
- **[P0] 辅助项重量可设 20kg 以下(59c66ae,2026-08-01)**:组录入弹窗的三条重量输入路径
  (初始值/步进器/数字键盘)此前对所有动作一律钳 20kg 下限(空杠假设),辅助项
  (哑铃/绳索/自重等,动作库约 1000 条)低于 20 的真实重量被静默改写成 20 落库。
  现按动作类型分流:辅助项下限 0,杠铃主项/变式保留 20kg 空杠护栏。学员内测反馈直接触发,
  **需要立即切包**。
