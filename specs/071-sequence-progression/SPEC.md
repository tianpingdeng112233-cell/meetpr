...
- **完成**(拍板 1,⚖️08-08 修订 auto 下线)= 服务端 completion 记录(manual:显式长按结算 /
...
- CTA「开始训练」交棒游标日(替换现行 `TodayWorkoutPlanHandoff(date: Date())` 的
  ...
  拍板(未开始态正典,⚖️08-08 三次收敛为最终口径):「今日训练」汇总卡 = 未开始态,
  **只属于零组记录的训练日**——CTA/「继续下一节」/直切训练 tab 进入 0 组日一律
  先见汇总卡,不自动开练,点「开始第一组」才进记录卡。**已开始的判据 = 点过
  「开始第一组」或已有 ≥1 组真实记录(含失败组,不含 assumed 补记)**;一旦已开始,
  重进(含杀进程冷启)直接恢复记录卡,不再回汇总卡。
  已完成日 ≠ 未开始态,它是只读详情(组表可见),判据=服务端 `completedAt`。**
...
- ~~**auto**:记满处方组后服务端自动完成~~(⚖️08-08 David 真机走查推翻:结算权归学员,
  服务端 auto 判定已随 backend 修订下线)。记满全部处方组后:剩余提示消失、长按按钮
  高亮引导结算;不长按则该日不推进(显式结算的接受代价)。客户端对 `completedAt` 的
  感知链路保留(manual 结算与教练 backfill 仍会产生 completion)。
...
同语义:completion store + manual/undo(⚖️08-08 起无 auto 判定);种子含「W1 前两天已完成、
...
- **游标推进全链**:记满 → 长按结算 → Dashboard/训练 tab 双端切态 → 次日 CTA 是下一天;
...
1. 完成判定 = **A**:~~全组记满自动完成 +~~ 显式「结束今天训练」结算,完成态落库(⚖️08-08 修订:auto 半边取消,长按=唯一学员侧结算,详见 §完成动作)。
