...
- 训练 tab 区块顺序固定为「今日训练」置顶、周编排列表在下;进入记录态后仍保留
  「今日训练」动作汇总块,当前动作高亮,允许收合但默认展开;周列表默认展开游标所在
  当前周,保证 D 行可见(David 2026-08-07 真机走查拍板)。
...
- 训练 tab 记录态保留默认展开、当前动作高亮的「今日训练」汇总,当前周默认展开;
