...
- **manual**(⚖️08-07 设计评审拍板,长按方案取代早稿的确认弹窗):仅在当天训练已开始
  (点过「开始第一组」或已有任意记录)后显示**长按「完成今日训练」**按钮;开始后即使
  仍为 0 组也允许长按跳过。旁侧常显剩余提示「还有 N 个动作 · M 组未记录」
  (全记满时该提示消失);长按本身即防误触,不再加确认弹窗。0 组也可长按(= 跳过这天,
  不造第二概念)→ `POST /plans/days/:dayId/complete`。按住过程必须有进度填充与触觉反馈;
...
- 手动完成入口仅在训练已开始后显示,长按有进度/触觉,提前松手有回弹反馈;
- 计划列表标题改「计划汇总」;
