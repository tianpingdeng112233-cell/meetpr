...
- 主态 = **游标日卡**:标题「W2D3」+ 日名/动作摘要,副标「教练推荐 8月5日」;
  Dashboard / 训练页标题 / 完成卡等所有训练日编码统一为 `W#D#` 连写,不加中间点
  (David 2026-08-07 真机走查拍板)。
  (推荐日期已过/未到都照实显示,不标红不催促——拍板 2 的语义就是「参考」)。
- CTA「开始训练」交棒游标日(替换现行 `TodayWorkoutPlanHandoff(date: Date())` 的
  日期交棒为 dayID 交棒);CTA 吸底常驻于 tab 栏上方并带训练日名副标,滚动内容为其
  预留空间,SE/mini 首屏必须可见(David 2026-08-07 真机走查拍板)。**⚖️08-08
  拍板:CTA 对零条非 assumed 真实日志的训练日只进入未开始态(今日训练卡 +
  「开始第一组」),不自动开练;已有真实日志的部分记录日恢复记录态;「继续下一节」
  同理不得让零记录日跳过未开始态。**
...
- 训练 tab 区块顺序固定为「今日训练」置顶、周编排列表在下;「今日训练」汇总卡
  **仅存在于未开始态**(动作列表 + 开始第一组),进入记录态后消失——记录态 =
  当前组大卡 + 分动作组表,两者不共存(⚖️David 2026-08-07 三次走查按原型钉死,
  取代同日早前「记录态保留汇总」的错误解读);周列表默认展开游标所在当前周
  (David 2026-08-07 真机走查拍板)。
...
- **manual**(⚖️08-07 设计评审拍板长按方案;⚖️08-07 David 二次真机走查收紧):
  **长按「完成今日训练」按钮仅在当天已有至少一组记录(含失败组)后出现**——记录前
  不显示、不置灰占位。**「跳过这天」概念不存在**:不练=游标停留即正确状态,想绕过
  某天=教练改计划,学员端不提供任何 0 组完成路径(backend 端点技术上仍接受 0 组,
  属 trust-client 面,iOS 永不触发)。旁侧常显剩余提示「还有 N 个动作 · M 组未记录」
  (全记满时该提示消失);长按本身即防误触,不再加确认弹窗
  → `POST /plans/days/:dayId/complete`。按住过程必须有进度填充与触觉反馈;
  未达时长松手必须可见回弹并给反馈,不得静默无反应(David 2026-08-07 真机走查拍板)。
...
- **游标推进全链**:记满 auto 完成 → Dashboard/训练 tab 双端切态 → 次日 CTA 是下一天;
  部分完成退出再进还是同一天;显式结束(≥1 组后)推进,0 组时长按按钮不可见;
  撤销回退游标并恢复可写;
...
- 不做教练端重定义(W2)、不做 plan-web 改动(W2)、不做 backend 顺延删除(W2,
  老包退场后);不做「今天没练」推送提醒;不做乱序选练(只能练游标日);
  不做「跳过这天」(⚖️08-07 拍板:不练=游标停留,绕过某天=教练改计划);
  不做同日多节硬门;不做推荐日期的动态顺推(拍板 2 已否)。
- **学员端无「休息日」概念**(⚖️08-07 设计评审拍板):今日卡在任何日历日打开都显示
  游标日(「下一节 W几D几 · 教练推荐 X/X」),不存在「今天是休息日 · 无训练安排」态
  ——那是日历制残余心智。计划浏览在训练 tab 序列列表,其标题正典文案:
  「计划汇总」,副行保留「已完成 X / Y 节」(David 2026-08-07 真机走查拍板)。
...
2. ~~手动完成 0 组也允许(= 跳过)~~(⚖️08-07 David 推翻:无跳过概念,手动完成需 ≥1 组记录);
...
- 设计稿正典文案采纳入 spec:训练 tab 计划列表标题「计划汇总」、未来日解锁说明
  「练完 W几 · ◯◯日 后自动轮到这一节」。
- **返修稿(2026-08-07 二审)已验收通过,入仓 `docs/design/sequence-handoff/` 为实装
  ...
  下一份计划由教练发布。发布后这里会直接出现 W1D1。」;手动完成按钮
  「长按 · 完成今日训练」+ 剩余提示「还有 N 个动作 · M 组未记录」。

### 真机走查返修(David 2026-08-07 拍板)

- `W#D#` 全端连写;
- 今日页「开始训练」吸底常驻,小屏首屏可见;
- 训练 tab「今日训练」汇总卡**仅存在于未开始态**(动作列表+开始第一组);记录态=
  当前组大卡+分动作组表,两者不共存(⚖️08-07 三次走查按原型钉死);当前周默认展开;
- 手动完成入口仅在**至少一组已记录(含失败组)后**显示(⚖️08-07 二次收紧,
  「点过开始第一组」不算),长按有进度/触觉,提前松手有回弹反馈;
- 计划列表标题改「计划汇总」;
- 训练 tab「今日训练」置顶,周编排列表下移。
