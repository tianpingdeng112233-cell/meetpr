...
- CTA「开始训练」交棒游标日(替换现行 `TodayWorkoutPlanHandoff(date: Date())` 的
  ...
  拍板(未开始态正典):「今日训练」汇总卡 = 未开始态的唯一形态,只要本次访问尚未
  点击「开始第一组」,进入该日永远显示这张卡——CTA/「继续下一节」/直切训练 tab
  一律如此,不自动开练;已有部分记录(中途退出/杀进程重进)同样先回汇总卡,
  点「开始第一组」后从第一个未记录组继续;存量日志只解锁长按完成门(进入记录态后),
  不得把界面快进到记录卡(取代同日早前「部分记录日恢复记录态」的口径)。
  该规则只管游标日:**已完成日 ≠ 未开始态**,它是只读详情(组表可见),
  判据=服务端 `completedAt`。**
